#!/bin/bash
# Validation script for pitchdeck.generate.v1

SKILL_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$SKILL_DIR"

echo "=== Pitchdeck Generator Validation ==="
echo ""
...
python3 -c "import sys; sys.path.insert(0, '../../..'); from skills.launchkit.pitchdeck.generate.v1.handler import run" 2>/dev/null
...
if [ $? -eq 0 ]; then
    ...
    SLIDES=$(echo "$RESULT" | python3 -c "import sys, json; print(json.load(sys.stdin).get('output', {}).get('total_slides', 0))" 2>/dev/null)
