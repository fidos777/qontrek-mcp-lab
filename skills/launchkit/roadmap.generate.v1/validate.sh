...
echo "=== Roadmap Generator Validation ==="
echo ""
...
python3 -c "import sys; sys.path.insert(0, '../../..'); from skills.launchkit.roadmap.generate.v1.handler import run" 2>/dev/null
...
if [ $? -eq 0 ]; then
    ...
    echo "   ✅ Status: $STATUS"
    MILESTONES=$(echo "$RESULT" | python3 -c "import sys, json; print(len(json.load(sys.stdin).get('output', {}).get('milestones', [])))" 2>/dev/null)
